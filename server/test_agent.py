...
async def cleanup_test_data(db: AsyncSession):
    ...
    result = await db.execute(
        select(Customer).where(
            Customer.phone_number.in_(["+1234567890", "+9876543210", None])
        )
    )
    ...
    result = await db.execute(
        select(Customer).where(
            Customer.email.in_(["test@example.com", "customer@test.com", None])
        )
    )
    customers = result.scalars().all()
    for customer in customers:
...
async def test_create_customer_telegram():
    ...
    async with AsyncSessionLocal() as db:
        ...
        print(
            f"✅ Created customer: ID={customer.id}, Telegram=@{customer.telegram_username}"
        )
        return customer.id
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if messages:
                    print(
                        f"   First message: {messages[0]['role']} - {messages[0]['content'][:50]}..."
                    )
            else:
                print(f"⚠️  API call returned status {response.status_code}")
