        ...
        for msg in message_data_list:
            ...
            if role != "unknown" and content:
                chat_messages.append(
                    {"role": role, "timestamp": timestamp, "content": content}
                )
...
async def toggle_ai(customer_id: int, db: AsyncSession = Depends(get_db)):
...
async def agent_reply(
    customer_id: int, request: AdminReplyRequest, db: AsyncSession = Depends(get_db)
):
    ...
    if customer.channel == "whatsapp" and customer.phone_number:
        ...
        chat_id = (
            customer.extra_data.get("chat_id")
            if isinstance(customer.extra_data, dict)
            else None
        )
        if not chat_id:
            raw_num = (
...
async def send_message_to_customer(
    ...
    try:
        ...
        response_data = await send_direct_message(
            db, request.customer_id, request.message
        )
